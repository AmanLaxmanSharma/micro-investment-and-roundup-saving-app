...
import { FiActivity, FiPlusCircle, FiArrowUpRight, FiArrowDownLeft, FiShoppingBag, FiInfo } from "react-icons/fi";
...
export default function TransactionsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      setAccounts(bankResponse.data.data.accounts || []);
    } catch (err) {
            ...
            <div>
              ...
              <select
                ...
                className={`block w-full px-3 py-2 bg-slate-900 border ${
                  errors.bankAccountId ? "border-rose-500" : "border-slate-800"
              ...
              {errors.bankAccountId && (
                <p className="mt-1 text-xs text-rose-500">{errors.bankAccountId.message}</p>
              )}
            </div>
